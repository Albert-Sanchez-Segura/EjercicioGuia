	...
	for(;;){
		...
		while (terminar == 0)
		{
			...
			if (codigo == 0)
				...
				sprintf(buff2, "%f grados Celsius son %f grados Farenheit", celsius, farenheit);
			}
			else if (codigo == 2){
				p = strtok(NULL, "/");
				float farenheit = atof (p);
				float celsius = (farenheit-32)/1.8;
				sprintf(buff2, "%f grados Farenheit son %f grados Celsius", farenheit, celsius);
			}
